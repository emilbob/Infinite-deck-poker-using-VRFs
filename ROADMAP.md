...
- [ ] M2: WASM + web demo — compile the engine to wasm, Vite + React 19 + Tailwind v4 page (no r3f/Lenis: no WebGL scene, no scroll narrative) where a game plays out with verification shown live, including a tamper panel that edits a transcript byte in-browser and shows the resulting error. Acceptance: `verify_transcript` runs in-browser on a game just played, and a tampered byte visibly fails.
